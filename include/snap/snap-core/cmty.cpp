...
// Stop when the Number of Connected Component Equals to a Set of Connected Component
double _CommunityGirvanNewman(PUNGraph& Graph, TCnComV& CmtyV, const unsigned int &k) {
  if (k <= 0)
  {
      printf("Wrong Input (Number of Ground Truth Comm) %d", k);
      return -1.0;
  }
  TIntH OutDegH;
  const int NEdges = Graph->GetEdges();
  ...
  TVec < TCnCom > CommV;
  TSnap::GetSccs(Graph, CommV);
  unsigned int l = CommV.Len();
  if (l >= k)
  {
      printf("Current number of Connected Component (%d) already match input k", l);
      return -1.0;
    //  return TSnap::GetModularity(Graph,CommV,-1); or simply return the modularity of existing connected component
  }
  while (true) {
    TSnapDetail::CmtyGirvanNewmanStep(Graph, Cmty1, Cmty2);
    const double Q = TSnapDetail::_GirvanNewmanGetModularity(Graph, OutDegH, NEdges, CurCmtyV);
    ...
    BestQ = Q;
    CmtyV.Swap(CurCmtyV);
    printf("current modularity: %f; current ConnectedComponent: %d\n", Q, CommV.Len());
    if (CommV.Len() >= k) { break; }
  }
  return BestQ;
}
